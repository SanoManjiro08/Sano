...
global.deku = new Object({
  ...
  AUTORESEND: config["SETTING"]["AUTORESEND"],
  TAG: config["TAG"],
});
...
global.handle = {
  replies: new Map(),
  cooldown: new Map(),
  // handleReaction: new Map(), // on next update lol
};
...
for (let files of fs.readdirSync(commandPath)) {
  if (files.endsWith(".js")) {
    ...
    try {
      ...
      logger("Installed command: " + script.config?.name);
    } catch (e) {
...
X(
  ...
  async function (err, api) {
    logs("THIS BOT WAS MADE BY SANO");
    logs("CONTACT: https://facebook.com/Mr.Grathegreat");
    logs("ADMIN: Mot Mot Oyamat");
    function getGUID() {
          ...
          const formData = {
            input: {
              ...
              audience: {
                privacy: {
                  allow: [],
                  base_state: "SELF", // SELF EVERYONE FRIENDS
                  deny: [],
    ...
    if (global.deku.AUTOCOMMENT == true) {
      cron.schedule(
        "*/1 * * * *",
        async function () {
          const pID = "07QJMSmf8rGgmweBSD9WZKJ6NgKuai2VLqwuCEh9s8kEn4bdsWPau3kyXismUShzwl"; // post ID
          const rands = [
            "Hi",
            "Hello",
            "Hey",
    ...
    api.listenMqtt(async function (err, event) {
      if (err) warn(err);
      // console.log(event);
      if (global.deku.AUTORESEND == true) {
          ...
          try {
            ...
            for (let newParticipant of addedParticipants1) {
              ...
              if (userID !== api.getCurrentUserID()) {
                api.shareContact(
                  `May bagong kupal nanaman sa ${threadName || "this group"}, pang ${participantIDs.length} na Kupal`,                      userID,
                  threadID,
                );
