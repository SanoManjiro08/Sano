...
function buildAPI(globalOptions, html, token, jar) {
  ...
  log.info("[ SANO LOGIN ] -", `Logged in as ${userID}`);
  ...
  if (oldFBMQTTMatch) {
    ...
    log.info("[ SANO LOGIN ] -", `Got this account's message region: ${region}`);
  } else {
    ...
    if (newFBMQTTMatch) {
      ...
      if (legacyFBMQTTMatch) {
        ...
        region = new URL(mqttEndpoint).searchParams.get("region").toUpperCase();
        log.warn("[ SANO LOGIN ] -", `Cannot get sequence ID with new RegExp. Fallback to old RegExp (without seqID)...`);
        log.info("[ SANO LOGIN ] -", `Got this account's message region: ${region}`);
        log.info("[ SANO LOGIN ] -", `[Unused] Polling endpoint: ${legacyFBMQTTMatch[6].split('",')[0]}`);
      } else {
        log.warn("[ SANO LOGIN ] -", "Cannot get MQTT region & sequence ID.");
        api.htmlData = html;
      }
    }
  }
                    ...
                    utils
                      ...
                      .then(function (res) {
                        try {
                          ...
                          log.info('[ SANO LOGIN ] -', 'Verified from browser. Logging in...');
                          var appState = utils.getAppState(jar);
                          return loginHelper(appState, email, password, loginOptions, cb);
                        }
                      })
                      .catch(function (err) {
                        log.error('[ SANO LOGIN ] -', err);
                        return cb(err);                        
                      });
                  }
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
  ...
  mainPromise
    .then(function () {
      log.info("[ SANO LOGIN ] -", 'Done logging in.');
